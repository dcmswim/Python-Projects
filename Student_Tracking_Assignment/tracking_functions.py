...
def center_window(self, w, h):
    #get screen width and height of user's device
...
#double checks if user wants to click when clicking 'x'
def ask_quit(self):
    if messagebox.askokcancel("Exit program", "Are you sure you want to exit application?"):
        self.master.destroy()
        os.exit(0) #prevents memory leak
...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_students (col_fname,col_lname,col_fullname,col_phone,col_email, col_course) VALUES (?,?,?,?,?,?)""", ('John','Doe','John Doe','111-111-1111','jdoe@email.com','Software Development'))
            conn.commit()
    conn.close()
