...
#This defines the GUI interface for this app
def load_gui(self):
    ...
    #WORKING ON FIXING 'COURSE' POSITIONING
    ...
    self.txt_course.grid(row=9,column=0,rowspan=1,columnspan=2,padx=(30,40),pady=(0,0),stick=N+E+W)
    #WORKING ON FIXING 'COURSE' POSITIONING
    ...
    self.lstList1.bind('<<ListboxSelect>>',lambda event: tracking_functions.onSelect(self,event))
    ...
    self.btn_add = tk.Button(self.master,width=12,height=2,text='Add',command=lambda: tracking_functions.addToList(self))
    self.btn_add.grid(row=10,column=0,padx=(25,0),pady=(45,10),sticky=W)
    self.btn_update = tk.Button(self.master,width=12,height=2,text='Update',command=lambda: tracking_functions.onUpdate(self))
    self.btn_update.grid(row=10,column=1,padx=(15,0),pady=(45,10),sticky=W)
    self.btn_delete = tk.Button(self.master,width=12,height=2,text='Delete',command=lambda: tracking_functions.onDelete(self))
    self.btn_delete.grid(row=10,column=2,padx=(15,0),pady=(45,10),sticky=W)
    self.btn_close = tk.Button(self.master,width=12,height=2,text='Close',command=lambda: tracking_functions.ask_quit(self))
    self.btn_close.grid(row=10,column=4,columnspan=1,padx=(15,0),pady=(45,10),sticky=E)

    
    tracking_functions.create_db(self)
    tracking_functions.onRefresh(self)
